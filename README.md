...
4. Откройте вкладку **ПОРТЫ** в нижней части окна Codespaces (рядом с плашкой **ТЕРМИНАЛ**).
5. Найдите порт `1984`.
6. Нажмите **открыть в браузере** перенаправляемый адрес.
7. Добавьте к адресу `/report.html` и перейдите по получившемуся адресу.

Итоговый адрес будет выглядеть примерно так:

```text
https://<название-codespace>-1984.app.github.dev/report.html
```

Всё! Отчёт открыт у вас в браузере через веб-сервер.
